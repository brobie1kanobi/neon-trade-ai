  ...
  useEffect(() => {
    ...
    const performRuleBasedTrade = async () => {
      ...
      try {
        ...
        // CRITICAL: Full trading mode - let the AI trade aggressively
        const isCashBuildUpMode = false; // Training wheels OFF
        let remainingCash = cashAvailable * 0.85; // Use 85% of available cash
        ...
        for (const p of prefs) {
          ...
          const rec = analysisMap[sym] || { confidence: 0.65, action: "buy" };
          if (rec.action === "sell") continue; // Only skip explicit sells, allow holds
          const basePct = Math.max(15, Number(p.percentage) || 15) / 100; // More aggressive base
          const multiplier = Math.min(2.0, 0.6 + rec.confidence); // Higher multiplier
          const fraction = Math.max(0.10, Math.min(0.40, basePct * multiplier)); // Larger positions
    ...
    // Run every 90 seconds for aggressive trading
