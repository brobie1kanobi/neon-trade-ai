...
export default function TradingInterface({ wallet, onTrade, autoTradingEnabled, holdings, isSimMode = true, currentCashBalance }) {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (!cancelled && userSettings.length > 0) {
          ...
          setAutoExecuteThreshold(userSettings[0].auto_execute_threshold ?? 70);
  ...
  const performLiveSearch = useCallback(async (term, type) => {
    ...
    try {
      ...
      const exchangeResults = Array.isArray(data) ? data.filter((asset) => type !== 'crypto' || asset?.source === 'kraken') : [];
      const symbols = [...new Set(exchangeResults.map((d) => d.symbol))];
