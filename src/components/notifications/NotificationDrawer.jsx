...
import { Bell, X, Info, AlertCircle, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function NotificationDrawer({ isOpen, onOpenChange }) {
  ...
  useEffect(() => {
    if (user) {
      Notification.filter({ created_by: user.email, read: false })
  ...
  const handleClearAll = async () => {
    try {
      ...
      while (batch.length === 100) {
        ...
        batch = await Notification.filter(
          { created_by: user.email, id: { $lt: lastId } },
          "-created_date",
          100
        );
  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={onOpenChange}>
        <SheetContent
          side="right"
          className="bg-slate-950 text-slate-100 p-0 fixed z-50 inset-y-0 right-0 h-full sm:max-w-sm w-[85vw] sm:w-[400px] flex flex-col border-l border-gray-800"
        >
        ...
        <DialogContent className="bg-slate-900 p-6 max-w-md">
          ...
          <div className="py-4 space-y-4">
            ...
            <p className="text-xs text-gray-400 text-right">
              {selectedNotification &&
                format(
                  new Date(selectedNotification.created_date),
                  "MMM d, yyyy h:mm a"
                )}
            </p>
          </div>
